...
function Install_ct()
{
    ...
    `wget --no-check-certificate http://tianzi77.github.io/vpn/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system`
    `mkdir /etc/gost && wget --no-check-certificate http://tianzi77.github.io/vpn/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost`
